...
import os
# from django.core.mail impozrt send_mail
...
def login(request):
    ...
    if request.method == "POST":
        ...
        try:
            user_list = qpuser.objects.get(userid=username)
            password_match= check_password(password,user_list.password)
            if password_match:
                ...
                content['err_msg'] = 'Invalid User Name or Password'
...
def index(request):
    ...
    cls = clubstatus.objects.filter(club_id__in=m).values('member','status')
    ...
    for b in cls.iterator():
        n.append(b['member'])
        if b['status'] == 'Pending':
            pending_list.append(b['status'])
            if b['member'] not in pendind_mems:
                pendind_mems.append(b['member'])

    pending_mems = member.objects.filter(id__in=pendind_mems).exclude(status='Inactive')

    my_queryset = member.objects.filter(id__in=n).exclude(status='Inactive')
    ...
    trans1 = visitor_logs.objects.filter(datetime__gte=date.today())
    ...
    content={'username':user.name,'count':qp_mem+nqp_mems,'pending_members':len(pending_list),'new_members':new_mem,'to_approve':pending_mems[:10],'renewals':renewals, 'clubs_list':userclubs,'qp_list':qp_mem,'nonqp_list':nqp_mems,'total_clubs':len(x),'clubutil':trns_list,'url':url}
...
def members(request):
    ...
    cls = clubstatus.objects.filter(club_id__in=m).values('member_id','status')
    n = deque()
    ...
    for b in cls.iterator():
        ...
        if b['status'] != 'Membership-Rejected':

            act_mem.append(b['member_id'])

        if b['status'] == 'Pending':

            pend_mem.append(b['member_id'])

        if b['status'] == 'Pending-Suspension':

            pend_sus.append(b['member_id'])

        if b['status'] == 'Pending-Cancel':

            pend_can.append(b['member_id'])

        if b['status'] == 'Suspended' or b['status'] == 'Membership-Suspended' or b['status'] == 'Pending-Renewal' or b['status'] == 'Rejected-Renewal' or b['status'] == 'Pending-Suspension-Revoke' or b['status'] == 'Membership-Rejected':
            oth.append(b['member_id'])
    
    all_mems=[]
    all_fams=[]
    all_gst=[]

    my_queryset = member.objects.filter(id__in=n).exclude(status='Inactive').values('id','name','rfidcardno','member_uid','dob','status')

    clubs = []
    active_members = my_queryset.filter(id__in=act_mem)#.values_list('member_id').distinct()
    result = []
    page = request.GET.get('page')
    
    try:
        for i in active_members.iterator(): 
            result.append(i)    
    ...
    if request.is_ajax():
        ...
        if 'page' in request.GET:
            ...
            active_members1 = my_queryset.filter(id__in=act_mem)
        ...
        if 'pend_page' in request.GET:
            ...
            pend_members = my_queryset.filter(id__in=pend_mem)
            if query is not None:
                page = query
            try:
        ...
        if 'sus_page' in request.GET:
            ...
            sus_members = my_queryset.filter(id__in=pend_sus)
            if query is not None:
                page = query
            try:
        ...
        if 'can_page' in request.GET:
            ...
            can_members = my_queryset.filter(id__in=pend_can)
            if query is not None:
                page = query
            try:
        ...
        if 'oth_page' in request.GET:
            ...
            # can_members = my_queryset.filter(status='Pending-Cancel').values('id','name','rfidcardno','member_uid','dob','status')
            can_members = my_queryset.filter(id__in=oth)
            if query is not None:
                page = query
            try:
                for i in can_members.iterator(): 
    ...
    suspend_members = my_queryset.filter(id__in=pend_sus)#.values('id','name','rfidcardno','member_uid','dob','status')
    ...
    pend_cancle = my_queryset.filter(id__in=pend_can)#.values('id','name','rfidcardno','member_uid','dob','status')
    ...
    others = my_queryset.filter(id__in=oth)
    ...
    content={'active_members':result,'sus_familylist':suspend_fmembers,'username' :user.name,'members':pend_page_result,'suspend_mem':sus_page_result,'pend_cancle':can_page_result,'others':oth_page_page_result,'url':'/members/'}
    if str(user.role) == 'Steward':
        return render_to_response('Steward/members.html',content,context_instance=RequestContext(request))  
...
def membershipform(request):
    ...
    if 'mid' in request.GET:
        ...
        family_obj = family.objects.filter(member_id=mem_obj.id).exclude(status='Inactive')
        for i in family_obj:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                memobj.member_uid = '900000' + str(memobj.id)
                ...
                if 'Imagefile' in request.FILES:
                    ...
                    data = f.read()
                    f.close()
                    f = open(os.path.join(conf_settings.IMAGE_ROOT, "%s.%s" %('900000' + str(memobj.id),flist[1])), "wb")
                    f.write(data)
                    f.close()
                if 'Imagefile' in request.FILES:
                    if f:
                        # memObj = member.objects.latest('id')
                        imageUrl  = "%s.%s" %('900000' + str(memobj.id),flist[1])
                        memobj.photo =  imageUrl
                ...
                if str(user.role) == 'Steward':
                    ...
                    subject = 'New request for membership access for Member Id :%s'%(memobj.member_uid)
                    message = 'Dear Supervisor,\n\n\n'
                    message += 'Please consider access request submitted for the Club Member whose details are as follows, \n\n\n'
                    name = memobj.name+' '+memobj.first_name_1
                    message += 'Member Name: %s \n\n\n '%name
                    message += 'Staff ID:  %s \n\n\n'%memobj.member_uid
                    message += 'Club: %s\n \n'%(str(lst))
                    message += 'Click http://localhost:8000/common/?mid=%s to take any action.'%(str(memobj.id))
...
def userform(request):
    ...
    for i in clubs_list:
        userclub1.append(i.name)  
    content = {'username' :user.name,'form':form, 'url':url, 'userclubs':userclub1}
    if request.method == "POST":
        ...
        if 'pwd' in request.POST:
            ...
            if uid:
                ...
                if user_form.is_valid():
                    ...
                    u_obj.password = pwd
                    u_obj.status = 'Active'
                    if u_obj.role.rolename=="Receptionist":
                        res_club=''
                        if 'res_club_edit' in request.POST:
                            res_club=request.POST['res_club_edit']
                        if not res_club:
                            if 'res_club' in request.POST:
                                res_club=request.POST['res_club'] 
                        cl = club.objects.filter(name__in=[res_club])       
                        u_obj.clubsallowed = cl
                    u_obj.save()
                    return HttpResponseRedirect('/users/')
            else:
                form=UserForm(request.POST)
                ...
                user_exist = qpuser.objects.filter(userid=str(usrid))
                if user_exist:
                    form=UserForm()
                    content.update({'form':form})
                    content['msg'] = 'User already exists with this Userid'
                ...
                if form.is_valid():
                    ...
                    # user_id=qpuser.objects.get(userid=usrname)
                        
                    obj = form.save()    
                    uobj = qpuser.objects.latest('id')
                    pwd = ''.join(random.choice('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ') for i in range(6))
                    uobj.password =make_password(pwd, salt=None, hasher='default') 
                    if uobj.role.rolename=="Receptionist":
                        res_club=request.POST['res_club']
                        uobj.clubsallowed =res_club
                    uobj.save()
    ...
    if 'uid' in request.GET:
        ...
        clubs=[]
        if u_obj.role.rolename=="Receptionist":
            content['is_receptionist']=1
            
            clubs.append(club.objects.filter(name__in=li)[0].name)
            
            for i in clubs_list:
                if i.name not in clubs:
                    clubs.append(i.name) 
            content['club']=clubs
        return render_to_response('Admin/userform.html',content, context_instance=RequestContext(request))
    ...
    if request.method == "POST":
        ...
        if filter == "daily":
            ...
            try:
                ...
                if user.role.rolename=="Steward":
                    return render_to_response('Steward/reports.html',content,context_instance=RequestContext(request))
                else:    
            ...
            try:
                tr=visitor_logs.objects.filter(datetime__range=(fromdate, todate))
            except:
                msssg="The Date You Entered Is Invalid"
                content['msssg']=msssg
        ...
        if filter=="weekly":
            ...
            dateDiff=todt-frmdt
            tr=visitor_logs.objects.filter(datetime__range=(frmdt, todt))
            m=0
            trxn_list=[]
            # print "Baaaadddaddaaa bachhaaa",family_adult
        ...
        if filter=="hourly":
            ...
            trans_list=visitor_logs.objects.filter(datetime__range=(datetime_start,datetime_end))
    ...
    if request.method == "GET":
        ...
        if 'export' in request.GET:
            ...
            if filter=="daily":
                ...
                tr=visitor_logs.objects.filter(datetime__range=(fromdate, todatee))
                ...
                if type == 'excel':
                    ...
                    ws.write(1,1,"(Reports For: From Date="+" "+fromdate+", To Date="+" "+todate+", Club="+club_q+")")
            ...
            if filter=="weekly":
                ...
                if type == 'excel':
                    ...
                    font_style.font.bold = True
                    ws.write(0,2,filename, font_style)
                    ws.write(1,1,"(Reports For: Month="+" "+moonth+", Year="+" "+year+", Club="+club_q+")")
                    for col_num in xrange(len(columns)):
                        ws.write(row_num, col_num, columns[col_num][0], font_style)
                        # set column width
                        ws.col(col_num).width = columns[col_num][1]
            ...
            if filter=="yearly":
                ...
                if type == 'excel':
                    ...
                    ws.write(1,1,"(Reports For: Year From="+" "+yearf+", Year To="+" "+yeart+", Club="+club_q+")")
                    for col_num in xrange(len(columns)):
                        ws.write(row_num, col_num, columns[col_num][0], font_style)
                        # set column width
                        ws.col(col_num).width = columns[col_num][1]
            ...
            if filter=="hourly":
                ...
                if type == 'excel':
                    ...
                    font_style.font.bold = True
                    ws.write(0,2,filename, font_style)
                    ws.write(1,1,"(Reports For: From Time="+" "+timefrom+", To Time="+" "+timeto+", Club="+club_q+")")
                    for col_num in xrange(len(columns)):
                        ws.write(row_num, col_num, columns[col_num][0], font_style)
                        # set column width
                        ws.col(col_num).width = columns[col_num][1]
...
def familyform(request):
    ...
    if 'mid' in request.GET:
        mid = request.GET['mid']
        family_members = family.objects.filter(member=mid).exclude(status='Inactive')
        ...
        if 'mid' in request.POST:
            ...
            if not fid:
                ...
                if 'Imagefile' not in request.FILES:
                    ...
                    f_obj.save()
                f_obj = family.objects.get(id=f_newid[0].id)
                fmaily_count=family.objects.filter(member=mem_obj.id).count()
                #fmaily_count+=1
                f_obj.family_uid = str(mem_obj.member_uid)+'000'+str(fmaily_count)
                ...
                if age >= 21 and f_obj.relationship != 'W' and not f_obj.date_of_expiry and f_obj.relationship != 'M' and f_obj.relationship != 'F' and f_obj.relationship != 'H':
                    f_obj.status = 'Pending-Crossed-Age 21'

                if f_obj.relationship == 'H' or f_obj.relationship == 'W' or f_obj.relationship == 'F' or f_obj.relationship == 'M':
                ...
                for i in range(len(c)):
                    ...
                    if(c[i] == famid):                
                        ...
                        if year >= 30:
                            ...
                            if relationship == 'M' or relationship == 'F':
                        ...
                        if int(year1) > datetime.now().year:

                            year1 = '19' + year
 
                        dob=date(int(year1),mt,int(day))
                        ...
                        if not fm_id:
                            ...
                            family_count=family.objects.filter(member=memobj.id).count()
                            fobj.family_uid = str(memobj.member_uid)+'000'+str(family_count)
                            days_in_year = 365.2425    
                            age = int((date.today() - fobj.dob).days / days_in_year)
                            if age < 0:
                                age = -1 * age
                            fobj.age = age
                            if age >= 21 and fobj.relationship != 'W' and not fobj.date_of_expiry and fobj.relationship != 'M' and fobj.relationship != 'F' and  fobj.relationship != 'H':
                                fobj.status = 'Pending-Crossed-Age 21'
...
def search(request):
    ...
    if request.method == 'POST':
        ...
        if search_text:
            mems = member.objects.filter(Q(name__icontains=search_text)|Q(member_uid__icontains=search_text))[:20]
...
def nearrenewal(request):
    ...
    if request.method == "POST":
        ...
        content['club_q']=club_q
        if club_q=='All':
            renew_mem=clubstatus.objects.filter(date_of_expiry__isnull=False).exclude(status='Inactive').exclude(status='Pending')
        else:
            club_id=club.objects.get(name=club_q)
            renew_mem=clubstatus.objects.filter(club=club_id,date_of_expiry__isnull=False).exclude(status='Inactive').exclude(status='Pending')
        for i in renew_mem:
            if i.date_of_expiry != None:
    ...
    if request.method == "GET":
        if 'export' in request.GET:
            ...
            if typ == 'excel':
                ...
                ws.write(1,1,"(Reports For: Club= "+club_q+")", font_style1)
                for col_num in xrange(len(columns)):
                    ws.write(row_num, col_num, columns[col_num][0], font_style)
                    # set column width
                    ws.col(col_num).width = columns[col_num][1]
...
def attendancereport(request):
    ...
    if request.method == "POST":
        ...
        if not obj_listm and not obj_listf:    
            msg="This Staff ID/Membership ID Does Not Exist"
            content['msg']=msg
            return render_to_response('attendancereport.html',content,context_instance=RequestContext(request))
        ...
        if obj_listm:    
            ...
            try:
                obj_list= visitor_logs.objects.filter(rfidcardid=card_id,datetime__range=(fromdate, todate))
            except:
                msssg="The Date You Entered Is Invalid"
                content['msssg']=msssg
                return render_to_response('attendancereport.html',content,context_instance=RequestContext(request))
    ...
    if request.method == "GET":
        if 'd1' in request.GET:
            ...
            if typ == 'excel':
                ...
                font_style1 = xlwt.XFStyle()
                font_style1.font.bold = False
                ws.write(1,0,"(Reports For: From Date="+" "+fromdate+" To Date="+" "+tdate+" Staff ID/Membership ID= "+member_id +" Club="+club_q+")", font_style1)
                for col_num in xrange(len(columns)):
                    ws.write(row_num, col_num, columns[col_num][0], font_style)
                    # set column width
                    ws.col(col_num).width = columns[col_num][1]
        ...
        if 'd1' in request.GET:
            ...
            if typ == 'excel':
                ...
                if club_q=="All":
                    ...
                    font_style1 = xlwt.XFStyle()
                    font_style1.font.bold = False
                    ws.write(1,1,"(Reports For: From Date="+" "+fromdate+" To Date="+" "+tdate+" Club="+club_q+")", font_style1)
                    for col_num in xrange(len(columns)):
                        ws.write(row_num, col_num, columns[col_num][0], font_style)
                        # set column width
                        ws.col(col_num).width = columns[col_num][1]
...
def suspensionrevokereport(request):
    ...
    if request.method == "POST":
        ...
        if club_q=='All':
            try:
                obj_listm = suspension.objects.filter(datetime__range=(fromdate, todate),status='Suspension-Revoke')
            except:
                msssg="The Date You Entered Is Invalid"
                content['msssg']=msssg
                return render_to_response('suspensionrevokereport.html',content,context_instance=RequestContext(request))       
            ...
            club_id=club.objects.get(name=club_q)
            try:
                obj_listm = suspension.objects.filter(datetime__range=(fromdate, todate),club=club_id,status='Suspension-Revoke')
            except:
                msssg="The Date You Entered Is Invalid"
                content['msssg']=msssg
                return render_to_response('suspensionrevokereport.html',content,context_instance=RequestContext(request))   
    ...
    if request.method == "GET":
        if 'd1' in request.GET:
            ...
            if typ == 'excel':
                ...
                font_style1 = xlwt.XFStyle()
                font_style1.font.bold = False
                ws.write(1,1,"(Reports For: From Date= " +str(fromdate)+", To Date="+str(tdate)+", Club="+club_q+")", font_style1)
                for col_num in xrange(len(columns)):
                    ws.write(row_num, col_num, columns[col_num][0], font_style)
                    # set column width
                    ws.col(col_num).width = columns[col_num][1]
...
def guestEntry(request):
    ...
    if request.method == 'POST':
        ...
        if 'gid' in request.POST:
            ...
            if gid:
                ...
                residencelocation=request.POST['residencelocation']
                phone_no=request.POST['phone_no'] 
                nationality=request.POST['nationality']
                guest_obj = guest.objects.get(id=gid)
                guest_obj.rfidcardno=rfidcardno
                guest_obj.name=name
                guest_obj.gender=gender
                guest_obj.dob=dob
                guest_obj.phone_no=phone_no 
                guest_obj.residencelocation=residencelocation  
                guest_obj.nationality=nationality
                guest_obj.emailid=emailid
            ...
            if not gid:
                ...
                guest_form = GuestForm(request.POST)
                if guest_form.is_valid():
                    clubs1 = guest_form.cleaned_data['clubsallowed']
                    guest_form.save()
                    # print guest_form
                    ...
                    print "dddddddddddddddddddaaaaaaaa" ,day
                    ...
                    subject = 'Alert for New Guest Profile in QP Club Management'
                    message = 'Dear Supervisor,\n\n\n'
                    message += 'New Guest Profile Has Been Created By:%s,\n'%(user.name)
                    message += 'Guest Name: %s,\n'%(guest_obj.name)
                    message += 'Club : %s,\n'%(userclubs[0])
                    message += 'Valid For : %s Month '%(guest_obj.period)
                    # 
                    from_email = 'venugopal@techanipr.com'
                    # to_list = ['ashok@indiadens.com']
                    # return HttpResponse(message)
    ...
    if request.method == 'GET':
        ...
        if 'gid' in  request.GET:
            ...
            content['member'] = guest_obj.memberid
...
def guestList(request):
    ...
    mems = member.objects.filter(id__in=guest_n).exclude(status='Inactive').values('member_uid')
...
def print_card(request):
    ...
    if 'fid' in request.GET:
        ...
        p.drawInlineImage(back_logo, 120,600, width=245.0,height=155.0)
        p.setFont('Helvetica-Bold',11,leading=None)
        p.drawCentredString(230,740,"Club Membership")
        seal = conf_settings.IMAGE_ROOT + "%s" %(fmem_obj.photo)
        try:
            p.drawImage(seal,125,660, width=50.0, height=50.0)
        except:
            pass
            msg="Please provide Image In JPG Format."
        p.setFont('Helvetica',10,leading=None)
        p.drawCentredString(200,700,"Name:")
        p.drawCentredString(300,700,fmem_obj.dapendent_first_name1+' '+fmem_obj.dependent_family_name)
        p.drawCentredString(220,670,"Membership ID:")
        p.drawCentredString(290,670,fmem_obj.family_uid)
        p.drawCentredString(215,650,"Relationship:")
        if fmem_obj.relationship:
            p.drawCentredString(312,650,fmem_obj.relationship)

        back_side_id = path+'/static/QP_id_back.jpg'
        p.drawInlineImage(back_side_id, 120,400, width=245.0,height=155.0)
        p.showPage()
        p.save()
    ...
    if 'id' in request.GET:
        ...
        p.setFont('Helvetica',10,leading=None)
        p.drawCentredString(200,700,"Name:")
        p.drawCentredString(290,700,mem_obj.name +' '+ mem_obj.first_name_1)#+ ' ' + mem_obj.first_name_3)
        p.drawCentredString(220,670,"Membership ID:")
        p.drawCentredString(290,670,mem_obj.member_uid)
        p.drawCentredString(200,650,"Grade:")
        if str(mem_obj.membership_grade):
            p.drawCentredString(290,650,str(mem_obj.membership_grade))
        p.drawCentredString(212,630,"Department:")
        if str(mem_obj.membership_grade):
            p.drawCentredString(290,630,str(mem_obj.department))
        # back_side_id = path+'/static/QP_id_back.jpg'
        # p.drawInlineImage(back_side_id, 120,400, width=245.0,height=155.0)
        # p.drawCentredString(200,610,"Club")
        # print "mem_obj.clubsallowed",mem_obj.clubsallowed
        # clubs = str(mem_obj.clubsallowed)
        # print clubs
        # print len(clubs)
        # p.drawCentredString(300,610,mem_obj.clubsallowed)
...
def display(request):
    ...
    trans = visitor_logs.objects.filter(club=cid)
    ...
    if trans:
        ...
        if flashid:
            ...
            fids = deque()
            display = ''
            mem_card_ids = clubstatus.objects.filter(club_id=cid,member__rfidcardno=flashid).exclude(status='Inactive').select_related("member_id")#member.objects.values('rfidcardno','clubsallowed')   
            ...
            if mem_card_ids:
                ...
                if st == "":
                    ...
                    content['status']=st
                    content['uname']=name
                    content['dependents'] = family.objects.filter(member_id=mem_card_ids[0].member.id).exclude(status='Inactive').count()
            ...
            family_ids = family.objects.filter(rfidcardno=flashid).exclude(status='Inactive')
            ...
            if guest_ids:                              
                ...
                if str(flashid).lower() in gids:            
                    ...
                    if st=="":
                        ...
                        #content['expirydate']=i.date_of_expiry
                        content['uid']=guest_ids[0].memberid
...
def members_visit(request):
    ...
    all_guest = guest.objects.all().exclude(status='Inactive')
    trans1=[]
    try:
        trans1 = visitor_logs.objects.filter(datetime__gte=date.today(),club_id=cl[0])
    except:
        pass    
...
def childreenage(request):
    ...
    if request.method == "POST":
        ...
        if club_q=="All":
            ...
            x =  club.objects.filter(name=club_q)
        m = []
        for k in x:
            m.append(k.id)
        cls = clubstatus.objects.filter(club_id__in=m).values('member_id')
    ...
    if request.method == "GET":
        if 'export' in request.GET:
            ...
            if typ == 'excel':
                ...
                font_style1 = xlwt.XFStyle()
                font_style1.font.bold = False
                ws.write(1,2,"(Reports For: Club="+club_q+")", font_style1)
                for col_num in xrange(len(columns)):
                    ws.write(row_num, col_num, columns[col_num][0], font_style)
                    # set column width
                    ws.col(col_num).width = columns[col_num][1]
...
def qpaddmember(request):
        ...
        if emid:
            ...
            if member_form.is_valid():
                ...
                if 'Imagefile' in request.FILES:
                    ...
                    data = f.read()
                    f.close()
                    print 'file',conf_settings.IMAGE_ROOT
                    f = open(conf_settings.IMAGE_ROOT + "%s.%s" %(memberedit.member_uid,flist[1]),"wb")
                    # size = (100,100)
                    # f.thumbnail(size, Image.ANTIALIAS)
                    # f.save(outfile, "JPEG")
    ...
    if request.method == 'POST':
        if 'personid' in request.POST:
            ...
            organization = '1'
            employment = '2'
            mem_id = member.objects.filter(member_uid=member_uid)
            if mem_id:
                # if mem_id[0].status != 'Inactive':
                    err_msg = 'Member already Added !!'
                    form = MemberForm(user=userid)
                    content = {'username':userid.name,'err_msg':err_msg,'form':form}
                    return HttpResponse('<script type="text/javascript">alert("Member already added !");window.opener.parent.location.href = "/membershipform/";</script>')

            form=MemberForm(initial={'name': name, 'first_name_1' : first_name_1, 'first_name_2' : first_name_2, 'first_name_3' : first_name_3,    'initials' : initials,    'office_fax_no' : office_fax_no, 'cont_type' : cont_type, 'cont_term_reason' : cont_term_reason, 'cont_expiry_date' : cont_expiry_date,    'No_of_dependents' : No_of_dependents,    'member_uid': member_uid, 'nationality': nationality, 'dob': dob, 'mobileno': mobileno, 'gender': gender, 'maritalstatus': maritalstatus, 'residencelocation':residencelocation, 'membership_grade': membership_grade, 'membership_category': membership_category, 'associatecompany':associatecompany, 'status': status, 'employment': employment, 'organization': organization, 'department': department, 'qpuser': user, 'extract_run_date':extract_run_date, 'pos_desc':pos_desc, 'worklocation':worklocation},user=userid)
            ...
            if form.is_valid():
                ...
                #     # customer.save()
                obj.save()
                memobj = member.objects.filter(qpuser=userid.id).exclude(status='Inactive').latest('id')
                clubstatus_obj = clubstatus()
                # date = datetime.now()
                # # date_of_joining = "%s-%s-%s" % (date.year, date.month, date.day)
                # date_of_expiry = "%s-%s-%s" % (date.year+1, date.month, date.day)
        ...
        if form.is_valid:
            ...
            if 'Supervisor' not in str(user.role):
                ...
                if status == 'Pending-Suspension':
                    ...
                    message += 'Staff ID:  %s \n\n\n'%mem_obj.member_uid
                    message += 'Club: %s\n \n'%(str(lst))
                    message += 'Click http://10.73.0.58:8000/common/?mid=%s to take any action.'%(str(mem_obj.id))
                    from_email = 'venugopal@techanipr.com'
                    # to_list = ['ashok@indiadens.com']
                    # return HttpResponse(message)
                    datatuple = (
...
def reset_self_pwd(request):
    ...
    if request.method == 'POST':
        ...
        encripted_password=make_password(confirmpassword)
        userobj.password = encripted_password
        userobj.save()
...
def family_suspension(request):
    ...
    if 'fid' in request.POST:
        ...
        post_date = datetime.strptime(str(date1) , '%Y-%m-%d')
        if post_date < datetime.now():
            return HttpResponse('<script type="text/javascript">alert("Date must be in future!!");window.close();window.opener.location.reload(true);</script>')
...
def view_profile(request):
    ...
    if request.method == 'POST':
        ...
        if search_text:
            ...
            if not mems:
                #print request.url
                return HttpResponse('<script type="text/javascript">alert("Member Does not exists !");window.location.href ='+current_url+';</script>')
            if mems:    
...
def view_profile_guest(request):
    ...
    if request.method == 'POST':
        search_text = request.POST['search_members']
        search_msg="Please Enter Text To Search"  
        ...
        if search_text:
            ...
            if gst:
                ...
                return render_to_response('Receptionist/guestList.html',content, context_instance=RequestContext(request))
        return render_to_response('Receptionist/guestList.html',content, context_instance=RequestContext(request))
    #return HttpResponseRedirect('/guestL
def rfid_post(request):
    ...
    if request.method == 'GET':
        ...
        if 'rfid' in request.GET and 'cid' in request.GET:
            ...
            tr_obj = visitor_logs(rfidcardid=rfid,club_id=cid,datetime=datetime.now())
...
def member_upload(request):
    ...
    if request.method == 'POST':
        ...
        for i in list1:
            list2.append(i.member_uid)
        if request.FILES:
            input_excel = request.FILES['excel']
            print input_excel.name
            # book = xlrd.open_workbook(file_contents=input_excel.read())
            # sheet = book.sheet_by_name("Qpdata")

            workbook = xlrd.open_workbook(file_contents=input_excel.read())
            sheet = workbook.sheet_by_index(0)
            for r in range(1, sheet.nrows):
                personal_number = sheet.cell(r,1).value
                family_name = sheet.cell(r,2).value
                firstname1 = sheet.cell(r,3).value
                firstname2 = sheet.cell(r,4).value
                firstname3 = sheet.cell(r,5).value
                initials = sheet.cell(r,6).value
                arabic_name = sheet.cell(r,7).value
                sex = sheet.cell(r,8).value
                maritalstatus = sheet.cell(r,9).value
                primenationality = sheet.cell(r,10).value
                department = sheet.cell(r,11).value
                worklocation = sheet.cell(r,12).value
                direct_off_phone = sheet.cell(r,13).value
                office_fax_no = sheet.cell(r,14).value
                personal_level = sheet.cell(r,15).value
                pos_descr = sheet.cell(r,16).value
                cont_type = sheet.cell(r,17).value
                cont_term_reason = sheet.cell(r,18).value
                cont_expiry_date = sheet.cell(r,19).value
                number_of_dependents = sheet.cell(r,20).value
                active_status = sheet.cell(r,21).value
                extract_run_date = sheet.cell(r,22).value
                status = 'Pending'
                list3.append(personal_number)
                if personal_number in list2:
                    pass


                else:
                    # demoupdate = member.objects.create(member_uid=personal_number,name=family_name,first_name_1=firstname1,first_name_2=firstname2,first_name_3=firstname3,initials=initials,arabic_name=arabic_name,gender=sex,maritalstatus=maritalstatus,nationality=primenationality,department=department,worklocation=worklocation,mobileno=direct_off_phone,office_fax_no=office_fax_no,membership_category=personal_level,pos_desc=pos_descr,cont_type=cont_type,cont_term_reason=cont_term_reason,cont_expiry_date=cont_expiry_date,No_of_dependents=number_of_dependents,extract_run_date=extract_run_date,status=status,qpuser_id=1)
                    demoupdate = member.objects.bulk_create([member(member_uid=personal_number,name=family_name,first_name_1=firstname1,first_name_2=firstname2,first_name_3=firstname3,initials=initials,gender=sex,maritalstatus=maritalstatus,nationality=primenationality,department=department,worklocation=worklocation,mobileno=direct_off_phone,office_fax_no=office_fax_no,membership_category=personal_level,pos_desc=pos_descr,cont_type=cont_type,cont_term_reason=cont_term_reason,cont_expiry_date=cont_expiry_date,No_of_dependents=number_of_dependents,extract_run_date=extract_run_date,status=status,qpuser_id=1,organization_id=1,datetime=datetime.now(),photo='people.jpg')])
                    print 'member',personal_number
            for i in list2:
                if i in list3:
                    z = str(i)
                    demo_obj = member.objects.get(member_uid = z)
                    if demo_obj.status == 'Inactive':
                        demo_obj.status = 'Pending'
                        demo_obj.save()
                else:
                    list5.append(i)
            for i in list5:
                z = str(i)
                demo_obj = member.objects.get(member_uid = z)
                fam_obj=family.objects.filter(member=demo_obj.id).update(status="Inactive")
                print demo_obj.status
                demo_obj.status = 'Inactive'
                demo_obj.save()

            message = 'File Uploaded Successfully'
            content['message'] = message

    return render_to_response('Admin/member_upload.html',content, context_instance=RequestContext(request))
...
def family_upload(request):
    ...
    list3 = []
    list5 = []
    list1 = family.objects.all().exclude(status='Inactive')
    if list1:
        content['family_db']=1
    for i in list1:
        list2.append(i.family_uid)
    ...
    if request.method == 'POST':
        if request.FILES:
            ...
            for r in range(1, sheet.nrows):
                ...
                z = str(sheet.cell(r,1).value)
                # print z
                demo_obj =  member.objects.filter(member_uid = z)
                if demo_obj:
                    demo_obj = demo_obj[0]
                    fam_obj = family.objects.filter(member=demo_obj.id).count()
                    fam_obj += 1
                    mem_obj = demo_obj
                    family_uid = str(demo_obj.member_uid)+'000'+str(fam_obj)
                    dependent_first_name1 = sheet.cell(r,2).value
                    dependent_family_name = sheet.cell(r,3).value
                    dependent_type = sheet.cell(r,4).value
                    dependent_birthdate = sheet.cell(r,5).value

                    dependent_birthdate = dependent_birthdate.split('-')
                    date1 = dependent_birthdate[0]
                    month = dependent_birthdate[1]
                    year = int(dependent_birthdate[2])
                    year_to_concat = dependent_birthdate[2]

                    if year >= 30:

                        if dependent_type == 'M' or dependent_type == 'F':

                            year1 = '19' + year_to_concat

                        else:
                            year1 = '20' + year_to_concat
                    ...
                    DEPENDENT_DOB = year1 + '-' + month + '-' + date1

                    dob = datetime.strptime(str(DEPENDENT_DOB), '%Y-%m-%d').date()

                    # print dob
                    age = int((date.today() - dob).days / 365.2425)

                    if age >= 21 and (dependent_type == 'D' or dependent_type == 'S'):

                        status = 'Pending-Crossed-Age 21'

                    else:

                        status = mem_obj.status



                    dependent_sequence = sheet.cell(r,6).value
                    list3.append(family_uid)
                    try:
                        test_obj = family.objects.get(member_id=mem_obj.id,dapendent_first_name1=dependent_first_name1,dependent_family_name=dependent_family_name,dob=DEPENDENT_DOB)
                    except family.DoesNotExist:
                        test_obj = None
                    if test_obj:
                        pass
                    else:
                        dependentupdate = family.objects.bulk_create([family(member_id=mem_obj.id,age=age,nationality=mem_obj.nationality,photo='people.jpg',datetime=datetime.now(),family_uid=family_uid,dapendent_first_name1=dependent_first_name1,dependent_family_name=dependent_family_name,relationship=dependent_type,dob=DEPENDENT_DOB,dependent_sequence=dependent_sequence,status=status)])
                        print 'inserting',dependentupdate
                lag = time.time() - start_time
                print (lag/60),'minutes'
                message = 'File Uploaded Successfully in %s minutes'%(lag)
                content['message'] = message
    return render_to_response('Admin/family_upload.html',content, context_instance=RequestContext(request))

def download_workbook(request):
     lists = family.objects.all().exclude(status='Inactive')
     filename = "Qpfam"
     response = HttpResponse(content_type='application/ms-excel')
     response['Content-Disposition'] = 'attachment; filename='+filename+".xls"
     wb = xlwt.Workbook(encoding='utf-8')
     ws = wb.add_sheet("Qpfam")
     row_num = 0
     columns = [ 
      (u"PERSONNEL_NUMBERR", 4000),
      (u"FAMILY_UID", 4000),
      (u"DEPENDT_FIRST_NAME1",3000),
      (u"DEPENDT_FAMILY_NAME",6000),
      (u"DEPENDANT_TYPE", 1000),
      (u"DEPENDT_BIRTH_DATE", 3000),
      (u"DEPENDANT_SEQUENCE", 3000),
     ]
     style = xlwt.easyxf('font: bold 1')
     font_style = xlwt.XFStyle()
     font_style.font.bold = True
     ...
     for obj in lists:
      row_num += 1
      row = [str(obj.member.member_uid),
       str(obj.family_uid),
       obj.dapendent_first_name1,
       obj.dependent_family_name, 
       obj.relationship,
       obj.dob ,
       obj.dependent_sequence,
      ]
      for col_num in xrange(len(row)):
       ws.write(row_num, col_num, row[col_num], font_style)
     wb.save(response)
...
# @transaction.commit_on_success
# def test(request):
#     if transaction.features.supports_transactions:
#         print 'Support'
#     else:
