...
def memberData():
	...
	workbook = xlrd.open_workbook('Qpdata.xlsx')
	sheet = workbook.sheet_by_index(0)
	num_rows = sheet.nrows - 1
	num_cells = sheet.ncols - 1
	curr_row = 0
...
def countFamilyMembers():
	...
	for row in cursor.fetchall():
		...
		for row in cursor.fetchall():
			flag = flag + 1
			family_flag = member_id + str(flag)
...
def slaveToMasterMember():
    ...
    for row in cursor.fetchall():
        ...
        query = "INSERT INTO qpcmms_member (name , member_uid, first_name_1, first_name_2, first_name_3, initials, gender, maritalstatus, nationality, department, worklocation, mobileno, office_fax_no, membership_category, pos_desc, cont_type, cont_term_reason, cont_expiry_date, No_of_dependents, status, extract_run_date, qpuser_id, clubsallowed, datetime) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW())"
        cursor.execute(query, (name, member_id, first_name_1, first_name_2, first_name_3, initials, gender, maritalstatus, nationality, department, worklocation, mobileno, office_fax_no, membership_category, pos_desc, cont_type, cont_term_reason, cont_expiry_date, number_of_dependents, status, extract_run_date, qpuser_id, clubs_allowed))
...
def copyFamilyDataFromSlaveToMaster():
	...
	if(num_of_rows_family == 0):
		...
		for data in cursor.fetchall():
			#fetch db id and m7ember id from qpcmms_member, db_id is the primary key
			id = data[0]
			member_id = data[1]
			...
			for row in cursor.fetchall():
				family_uid = str(row[2])+'000'+str(row[0])
				id_slave_excel = row[0]
				member_id = row[2]
				club_code = row[1]
			...
			# print family_flag
				print "num_row: ", family_uid
				some_sql_statement = "INSERT INTO `qpcmms_family` (member_id,family_uid, dapendent_first_name1, dependent_family_name, dependent_sequence, relationship, dob, status, family_flag, time_flag) VALUES (%s,%s, %s, %s, %s, %s, %s,'Active', %s, CURDATE())"
				cursor.execute(some_sql_statement, (id,family_uid,dependent_first_name,dependent_family_name, dependent_sequence, relationship, date_of_birth, flag))
				print "Inserting Family: ", dependent_first_name


				sql_slave_update = "UPDATE `slave_excel_copy` SET `update_flag`=%s WHERE `f_member_uid`=%s AND `flag`=%s", ('1', member_id, flag)
				print sql_slave_update
				cursor.execute(*sql_slave_update)
				print "Updating: ", member_id, "with flag id: ", flag

				#Getting this error
				#(1452, 'Cannot add or update a child row: a foreign key constraint fails (`qpcm`.`qpcmms_family`, CONSTRAINT `member_id_refs_id_1ba5268f` FOREIGN KEY (`member_id`) REFERENCES `qpcmms_member` (`id`))')
